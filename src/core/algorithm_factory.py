...
class AlgorithmFactory(ABC):
    ...
    def run_algorithm(self, loading_screen: tk.Tk):
        ...
        while True:
            ...
            if equalmatrix(algorithm.image, algorithm.imgBeforeStep, algorithm.image.shape):
                break
            else:
                makeequalmatrix(
                    algorithm.imgBeforeStep, algorithm.image, algorithm.image.shape
                )

        algorithm.after_processing()
        save_image_by_name(algorithm.imgName, algorithm.image)
